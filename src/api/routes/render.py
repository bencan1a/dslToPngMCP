...
async def render_dsl_to_png(dsl_data: Dict[str, Any], options: Dict[str, Any]) -> PNGResult:
    ...
    if not parse_result.success:
        raise HTTPException(status_code=400, detail=f"DSL parsing failed: {'; '.join(parse_result.errors)}")
...
async def render_sync(request: DSLRenderRequest) -> RenderResponse:
    ...
    try:
        png_result = await render_dsl_to_png(request.dsl_content, request.options.dict())
...
async def render_async(request: DSLRenderRequest):
