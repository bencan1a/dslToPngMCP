...
from src.models.schemas import (
    DSLDocument, DSLElement, ParseResult, ElementType,
    ElementStyle, ElementLayout
)
...
class DSLValidator:
    ...
    def validate_document(self, data: Dict[str, Any]) -> tuple[bool, List[str], List[str]]:
        ...
        """
        validator = Validator(self.document_schema)
        validator.allow_unknown = True  # Allow extra fields
        ...
        if not is_valid:
            errors.extend(self._format_validation_errors(validator.errors))
    ...
    def _format_validation_errors(self, errors: Dict[str, Any], path: str = "") -> List[str]:
        ...
        for field, error_info in errors.items():
            ...
            if isinstance(error_info, list):
                for error in error_info:
                    formatted_errors.append(f"{current_path}: {error}")
            elif isinstance(error_info, dict):
                formatted_errors.extend(
                    self._format_validation_errors(error_info, current_path)
                )
    ...
    def _perform_custom_validations(self, data: Dict[str, Any]) -> tuple[List[str], List[str]]:
        ...
        if width > 1920 or height > 1080:
            warnings.append(f"Large canvas size ({width}x{height}) may impact performance")
    ...
    def _validate_element(self, element: Dict[str, Any], path: str) -> tuple[List[str], List[str]]:
        ...
        if not isinstance(element, dict):
            errors.append(f"{path}: Element must be a dictionary/object, got {type(element).__name__}: {repr(element)}")
            return errors, warnings
        ...
        container_types = {
            ElementType.CONTAINER, ElementType.GRID, ElementType.FLEX, ElementType.CARD,
            ElementType.NAVBAR, ElementType.SIDEBAR, ElementType.MODAL
        }
        if children and ElementType(element_type) not in container_types:
            errors.append(f"{path}: Element type '{element_type}' cannot have children")
        ...
        if element_type == ElementType.IMAGE.value and not element.get('src'):
            warnings.append(f"{path}: Image element should have 'src' property")
        
        if element_type == ElementType.BUTTON.value and not element.get('label'):
            warnings.append(f"{path}: Button element should have 'label' property")
...
class JSONDSLParser(BaseDSLParser):
    ...
    def __init__(self):
        self.logger = logger.bind(parser="json")
    ...
    async def _convert_to_dsl_document(self, raw_data: Dict[str, Any]) -> DSLDocument:
        ...
        for i, element_data in enumerate(raw_data.get('elements', [])):
            ...
            if not isinstance(element_data, dict):
                raise ValueError(f"Element at index {i} must be a dictionary/object, got {type(element_data).__name__}: {repr(element_data)}")
            element = await self._convert_to_dsl_element(element_data)
        ...
        return DSLDocument(
            ...
            responsive_breakpoints=raw_data.get('responsiveBreakpoints', {
    ...
    async def _convert_to_dsl_element(self, element_data: Dict[str, Any]) -> DSLElement:
        ...
        if 'layout' in element_data:
            ...
            layout = ElementLayout(
                ...
                min_width=layout_data.get('minWidth'),
                max_width=layout_data.get('maxWidth'),
                min_height=layout_data.get('minHeight'),
                max_height=layout_data.get('maxHeight')
            )
        ...
        for child_data in element_data.get('children', []):
            child = await self._convert_to_dsl_element(child_data)
        ...
        return DSLElement(
            ...
            custom_attributes=element_data.get('customAttributes', {}),
...
class YAMLDSLParser(BaseDSLParser):
    ...
    def __init__(self):
        self.logger = logger.bind(parser="yaml")
        self.validator = DSLValidator()
        ...
        try:
            ...
            if not isinstance(raw_data, dict):
                return ParseResult(
                    success=False,
                    document=None,
                    errors=[f"YAML content must be a dictionary/object, got {type(raw_data).__name__}"],
                    processing_time=time.time() - start_time
                )
            ...
            self.logger.debug("🔍 DIAGNOSTIC: YAML document validation",
                            is_valid=is_valid,
            ...
            document = await json_parser._convert_to_dsl_document(raw_data)
...
async def parse_dsl(content: str, parser_type: Optional[str] = None) -> ParseResult:
    ...
    if not content or not content.strip():
        return ParseResult(
            success=False,
            document=None,
            errors=["Empty DSL content provided"],
            processing_time=0.0
        )
...
async def get_validation_suggestions(content: str, errors: List[str]) -> List[str]:
    ...
    for error in errors:
        if "JSON syntax" in error:
            ...
            suggestions.append("Only container, grid, flex, and card elements can have children")
        elif "required" in error.lower():
            suggestions.append("Ensure all required fields are present: type for elements")
    
    # Content-based suggestions
    if "{" in content and "}" in content:
        if content.count("{") != content.count("}"):
            suggestions.append("Check for unmatched curly braces in JSON")
    
    if "elements" not in content:
        suggestions.append("DSL document should contain an 'elements' array")
    ...
    return unique_suggestions[:5]  # Limit to 5 most relevant suggestions
