...
class Jinja2HTMLGenerator(BaseHTMLGenerator):
    ...
    def __init__(self):
        ...
        self.logger = logger.bind(generator="jinja2")
    ...
    def _register_template_functions(self) -> None:
        ...
        def style_to_css(style: Optional[ElementStyle]) -> str:
            ...
            if style.font_size:
                css_rules.append(f"font-size: {style.font_size}{'px' if isinstance(style.font_size, int) else ''}")
            ...
            if style.border_radius:
                css_rules.append(f"border-radius: {style.border_radius}{'px' if isinstance(style.border_radius, int) else ''}")
            if style.margin:
                css_rules.append(f"margin: {style.margin}{'px' if isinstance(style.margin, int) else ''}")
            if style.padding:
                css_rules.append(f"padding: {style.padding}{'px' if isinstance(style.padding, int) else ''}")
        ...
        self.env.globals['render_element'] = render_element
    ...
    async def _prepare_context(self, document: DSLDocument, options: RenderOptions) -> Dict[str, Any]:
        """
    ...
    def _render_element_html(self, element: DSLElement, index: int = 0) -> str:
        ...
        # Layout styles
        if element.layout:
            layout_css = self.env.filters['layout_to_css'](element.layout)
            if layout_css:
                inline_styles.append(layout_css)
        
        # Element styles
        if element.style:
            style_css = self.env.filters['style_to_css'](element.style)
            if style_css:
        ...
        elif element.type.value in ["container", "grid", "flex", "card", "modal", "navbar", "sidebar"]:
    ...
    def _render_input(self, element: DSLElement, attributes: Dict[str, str]) -> str:
        ...
        attrs_str = self._build_attributes(attributes)
        placeholder = f' placeholder="{self._escape_html(element.placeholder)}"' if element.placeholder else ""
...
class ComponentHTMLGenerator(BaseHTMLGenerator):
    ...
    def __init__(self):
        self.logger = logger.bind(generator="component")
        self.component_registry = self._setup_component_registry()
    
    def _setup_component_registry(self) -> Dict[str, callable]:
    ...
    async def generate(self, document: DSLDocument, options: RenderOptions) -> str:
        ...
        try:
            ...
            html = '\n'.join(html_parts)
            self.logger.info("Component-based HTML generation completed",
                           html_length=len(html))
    ...
    def _render_container_component(self, element: DSLElement) -> str:
        ...
        style = self._build_element_style(element)
        children_html = "".join([self.component_registry.get(child.type.value, self._render_generic_component)(child) for child in element.children])
    ...
    def _render_modal_component(self, element: DSLElement) -> str:
        ...
        style = self._build_element_style(element) + "; background: white; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.15);"
        children_html = "".join([self.component_registry.get(child.type.value, self._render_generic_component)(child) for child in element.children])
    ...
    def _render_navbar_component(self, element: DSLElement) -> str:
        ...
        style = self._build_element_style(element) + "; background: #f8f9fa; border-bottom: 1px solid #e0e0e0; display: flex; align-items: center;"
        children_html = "".join([self.component_registry.get(child.type.value, self._render_generic_component)(child) for child in element.children])
    ...
    def _render_sidebar_component(self, element: DSLElement) -> str:
        ...
        style = self._build_element_style(element) + "; background: #f8f9fa; border-right: 1px solid #e0e0e0;"
        children_html = "".join([self.component_registry.get(child.type.value, self._render_generic_component)(child) for child in element.children])
    ...
    def _build_element_style(self, element: DSLElement) -> str:
        ...
        if element.style:
            ...
            if element.style.font_size:
                styles.append(f"font-size: {element.style.font_size}{'px' if isinstance(element.style.font_size, int) else ''}")
...
async def generate_html(document: DSLDocument, options: RenderOptions,
                       generator_type: str = "jinja2") -> str:
    """
    Generate HTML from DSL document.
