...
from src.models.schemas import DSLRequest, RenderOptions, PNGResult
...
class MessageHandler:
    ...
    async def handle_message(self, message: Union[Dict[str, Any], MCPMessage]) -> MCPResponse:
        ...
        try:
            ...
            self.logger.error("Message handling error", error=str(e), method=getattr(message, 'method', 'unknown'))
            return MCPResponse(
    ...
    async def _handle_resource_read(self, message: MCPMessage) -> MCPResponse:
        ...
        try:
            ...
            if resource_type not in self._resource_handlers:
                return MCPResponse(
                    error={"code": -32601, "message": f"Resource type not supported: {resource_type}"},
                    id=message.id
                )
    ...
    async def _render_dsl_tool(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            render_opts = RenderOptions(
                ...
                png_quality=options.get("png_quality", 95),
    ...
    def register_tool_handler(self, name: str, handler) -> None:
        """Register a custom tool handler."""
        self._tool_handlers[name] = handler
        self.logger.info("Registered tool handler", tool_name=name)
    
    def register_resource_handler(self, scheme: str, handler) -> None:
