...
from mcp.types import (
    ...
    LoggingLevel, ClientCapabilities, ServerCapabilities,
...
from src.models.schemas import (
    ...
    RenderOptions, TaskStatus, ErrorResponse, RenderResponse
)
...
class DSLToPNGMCPServer:
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent | ImageContent | EmbeddedResource]:
            ...
            try:
                ...
                if name == "render_ui_mockup":
                    return await self._handle_render_ui_mockup(arguments)
                elif name == "validate_dsl":
                    return await self._handle_validate_dsl(arguments)
                elif name == "get_render_status":
                    return await self._handle_get_render_status(arguments)
                else:
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> List[TextContent | ImageContent | EmbeddedResource]:
        """Call a specific MCP tool (public API)."""
        try:
            from mcp.types import CallToolRequest
            
            # Create proper JSON-RPC request with required fields and proper params
            ...
            # Get the handler from request_handlers
            handler = self.server.request_handlers.get(CallToolRequest)
            if handler:
                result = await handler(request)
                
                # Extract content from ServerResult object
                if hasattr(result, 'root') and hasattr(result.root, 'content'):
                    return result.root.content
                else:
                    # Fallback: return the result as-is if it's already the expected format
                    return result
            else:
                raise RuntimeError("CallToolRequest handler not found")
        except Exception as e:
            self.logger.error("Failed to call tool", tool=name, error=str(e))
            raise
    
    async def _handle_render_ui_mockup(self, arguments: Dict[str, Any]) -> List[TextContent | ImageContent]:
            ...
            if async_mode:
                ...
                try:
                    ...
                    if not parse_result.success:
                        ...
                        suggestions = await get_validation_suggestions(dsl_content, parse_result.errors)
                    ...
                    if "Browser pool not initialized" in error_msg:
                        ...
                        user_error = "Browser pool failed to initialize. Service temporarily unavailable."
                        error_code = "BROWSER_POOL_INITIALIZATION_FAILED"
                    elif "Browser pool exhausted" in error_msg or "No available browser" in error_msg:
                        user_error = "All browser instances are busy. Please try again in a moment."
                        error_code = "BROWSER_POOL_EXHAUSTED"
                    elif "timeout" in error_msg.lower():
                        user_error = "Browser operation timed out. Please try again with simpler content."
    ...
    async def run(self, transport_type: str = "stdio", host: str = "0.0.0.0", port: int = 3001) -> None:
        """Run the MCP server."""
        try:
            if transport_type == "stdio":
                ...
                async with stdio_server() as (read_stream, write_stream):
                    self.logger.info("MCP server starting with stdio transport", component="mcp_server")
                ...
                self.logger.info("MCP server starting with HTTP health check",
                               component="mcp_server", host=host, port=port)
